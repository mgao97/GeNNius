...
class GNN(nn.Module):
    def __init__(self, in_dim, n_hid, num_types, num_relations, n_heads, n_layers, dropout = 0.2, conv_name = 'hgt', prev_norm = True, last_norm = True):
    ...
    def forward(self, node_feature, node_type, edge_index, edge_type, cell_res):
...
class GNN_from_raw(nn.Module):
    ...
    def forward(self, node_feature, node_type, edge_index, edge_type):
        ...
        for t_id in range(self.num_types):
            ...
            # print(node_embedding_stack)
            # print('node_embedding_stack[idx]:',node_embedding_stack[idx].shape)
