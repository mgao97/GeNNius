...
class GAT(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels):
        ...
        self.lin = Linear(out_channels*2, out_channels)
    ...
    def forward(self, x_dict, edge_index, edge_label_index_dict):
        ...
        return torch.sigmoid(self.lin(torch.cat([x[edge_label_index_dict[('drug','interaction','protein')][0]], x[edge_label_index_dict[('drug','interaction','protein')][1]]], dim=1)))
...
#             conv = GATConv(hidden_channels, hidden_channels,add_self_loops=False)
...
path = 'Data/DAVIS/hetero_data_davis.pt'
...
# 初始化模型
model = GAT(hidden_channels, out_channels).to(device)
print('model:',model)
# 定义优化器
