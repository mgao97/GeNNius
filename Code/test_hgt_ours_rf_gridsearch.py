...
gridsearch = GridSearchCV(estimator = rf_model, param_grid = grid_search, 
...
for i in tqdm(range(run_time)):
    ...
    gridsearch.fit(train_edge_x_final, train_y)
    # end_time = time.time()
    # print(f"Elapsed time {(end_time-init_time)/60:.4f} min")

    final_rndfst = gridsearch.best_estimator_
    rf_model = final_rndfst.fit(train_edge_x_final, train_y)
