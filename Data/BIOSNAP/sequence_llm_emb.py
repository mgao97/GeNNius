...
# # 按行读取文件
# with open('sequence_llm.txt', 'rb') as file:
#     for line in file:
...
try:
    with open('sequence_llm.txt', 'r', encoding=encoding) as file:
        for line in file:
            parts = line.split(',', 1)  # Split only at the first comma
            if len(parts) > 1:  # Ensure there is content after the comma
                elements_list.append(parts[1].strip())  # Remove extra whitespace

except FileNotFoundError:
    print(f"Error: The file '{'sequence_llm.txt'}' was not found.")
except IOError as e:
...
# 保存为.pt文件
torch.save(tensor_data, 'sequence_llm_emb_64.pt')
