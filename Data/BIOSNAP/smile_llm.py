...
# 打开文件，准备追加内容
with open('smile_llm.txt', 'a', encoding='utf-8') as file:
    for i in range(0, len(df_SMILES), batch_size):
        # 获取当前批次的 SMILES 列表
        batch_smiles = df_SMILES[i:i + batch_size]
        ...
        # Skill:
            - 掌握蛋白质靶标相互作用，能够准确识别药物的SMILES和英文名称。
            - 输入的SMILES都是通过 https://pubchem.ncbi.nlm.nih.gov/ 网站数据库获得。
        # Task:输入药物SMILES，一句话介绍对应的SMILES的名称和包含的结构，按行输出,首先是SMILES，然后是SMILES结构特性（英文），SMILES和后面的结构特性用逗号隔开。
        # Constraints:不需要输出其他内容和有关大模型或其他网站的任何说明。。
        # Example:
        SMILES:C1=C(NC=N1)CCN，Imidazole, a basic organic compound containing a five-membered ring with two nitrogen atoms at positions 1 and 3, used as a starting material for synthesis in the pharmaceutical industry.

        # 输入:
        以下SMILES列表：{batch_smiles}
        """
