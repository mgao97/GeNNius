...
# unique drugs: drug={ x=[222, 12] },
# 复制原始tensor以确保随机选取不改变原始数据
exp_smile_llm_emb = torch.zeros(222,smiles_llm_emb.shape[1])
# 随机选择缺少的行
indices = torch.randperm(smiles_llm_emb.shape[0])[:222-smiles_llm_emb.shape[0]]
# 将选中的行复制到新的位置
exp_smile_llm_emb[smiles_llm_emb.shape[0]:222, :] = smiles_llm_emb[indices, :]


# unique proteins: protein={ x=[94, 20] },
exp_sequence_llm_emb = torch.zeros(94,smiles_llm_emb.shape[1])
indices = torch.randperm(41)[:94-sequences_llm_emb.shape[0]]
indices = torch.cat((indices,torch.randperm(12)[:94-sequences_llm_emb.shape[0]]),dim=0)
print(indices.shape)
exp_sequence_llm_emb[sequences_llm_emb.shape[0]:94, :] = sequences_llm_emb[indices, :]
# 初始化目标张量
